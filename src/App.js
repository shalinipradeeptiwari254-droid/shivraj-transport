          ...
          <div style={cardStyle}>
            ...
            {siteData.routes.map((r,i)=>(
              <div key={i} style={{display:"grid",gridTemplateColumns:"repeat(5,1fr) auto",gap:10,
                alignItems:"end",marginBottom:14,background:"#f9f9fc",padding:16,borderRadius:8,border:"1px solid #e8e8f0"}}>
                {[["from","From"],["to","To"],["km","Distance"],["days","Transit"],["rate","Rate"]].map(([k,l])=>(
                  <div key={k}>
                    <label style={LABEL}>{l}</label>
                    <input style={{...INP,marginBottom:0}} value={r[k]} onChange={e=>updateRoute(i,k,e.target.value)}/>
                  </div>
                ))}
                <button onClick={()=>delRoute(i)} style={btnRed}>🗑</button>
              </div>
            ))}
            <button onClick={()=>showSaved("Routes saved ✓")} style={{...btnNavy,padding:"10px 28px",fontSize:15}}>💾 Save</button>
            {saved && <span style={{marginLeft:14,color:"#27ae60",fontFamily:"'Lato',sans-serif",fontSize:14}}>{saved}</span>}
          </div>
        )}
...
export default function App() {
  ...
  const [siteData, setSiteData] = useState(DB.siteData);

  useEffect(()=>{ DB.siteData = siteData; },[siteData]);

  useEffect(()=>{
    const link = document.createElement("link");
    link.rel  = "stylesheet";
    link.href = "https://fonts.googleapis.com/css2?family=Oswald:wght@400;600;700&family=Lato:wght@300;400;700&display=swap";
  ...
  if(page==="Admin") return (
    <>
      ...
      <Admin siteData={siteData} setSiteData={setSiteData}/>
    </>
  );
  ...
  return (
    <div>
      ...
      <div style={{paddingTop:64}}>
        {page==="Home"          && <><Hero setPage={setPage} data={siteData}/><Services data={siteData}/><About data={siteData}/></>}
        {page==="Services"      && <Services data={siteData}/>}
        {page==="Freight Rates" && <FreightRates data={siteData}/>}
        {page==="About"         && <About data={siteData}/>}
        {page==="Contact"       && <Contact data={siteData}/>}
      </div>
